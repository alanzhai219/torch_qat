...
def run_qat(args):
    qat_model = load_model(os.path.join(args.model_path, args.model_file)).to('cuda')
    ...
    model_to_quantize = copy.deepcopy(qat_model)
    ...
    for nepoch in range(args.epochs):
        train_one_epoch(model_prepared, criterion, optimizer, data_loader, torch.device('cuda'), num_train_batches)
        lr_scheduler.step()
        quantized_model = quantize_fx.convert_fx(model_prepared, qconfig_dict)
        ...
        torch.jit.save(torch.jit.script(quantized_model), 'tmp_experiment.pt')
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--model-file',
                        default='alexnet-owt-7be5be79.pth',
